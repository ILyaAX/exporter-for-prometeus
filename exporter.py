...
hostname = subprocess.check_output(["cat", "/etc/hostname"]).decode("utf-8").rstrip()
ipinfo_out = json.loads(subprocess.check_output(["wget", "-qO-", "ipinfo.io"]).decode("utf-8"))
...
disk_total = int(subprocess.check_output(["df", "/"]).decode("utf-8").split()[8]) * 1024
...
host.labels(*METRICS_LABELS).info({'cpu_cores': cores, 'mem_total': mem_total, 'disk_total': str(disk_total), 'country': country, 'city': city, 'location': location})
